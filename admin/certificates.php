...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
            <span class="text-xs font-bold tracking-widest text-slate-400 uppercase">Accreditation Records</span>
            <h1 class="text-3xl font-light tracking-tight text-slate-900 mt-1">Certificates Management</h1>
        </div>
        ...
        <form method="GET" class="flex items-center gap-2 max-w-sm w-full md:w-auto">
            ...
            <?php if (!empty($search)): ?>
                <a href="certificates.php" class="px-3 py-2 text-xs font-medium text-slate-500 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg transition-colors">
                    Clear
                </a>
            <?php endif; ?>
        </form>
    ...
    <!-- Certificate Ledger Records Table Wrapper -->
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-200 text-[11px] uppercase tracking-wider text-slate-400 font-bold">
                        ...
                        <th class="py-3.5 px-6">Certificate Reference No</th>
                        <th class="py-3.5 px-6">Associated User Holder</th>
                        <th class="py-3.5 px-6">Timestamp Issued At</th>
                        <th class="py-3.5 px-6 text-right">Document Verification Asset</th>
                    </tr>
                </thead>
                    ...
                    <?php if (empty($certificates)): ?>
                        <tr>
                            <td colspan="5" class="py-10 px-6 text-center text-slate-400 italic">
                                <?php echo !empty($search) ? 'No matching accreditation records found for the search parameter.' : 'No issued membership certificates found in the ledger database archives.'; ?>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-50/40 transition-colors duration-150">
                            ...
                            <!-- Absolute Contextual File Mapping Action Link -->
